...
### 单文件处理页
- **顶部**：显示已选视频文件和时长
- **模式切换**：分辨率模式可选全帧/关键帧，数据库模式可选检测时段
...
**Q: 切出来的视频不完整/画质变差？**
分辨率模式采用 `-c copy` 无损截取，不重新编码，画质无损。如果检测结果不理想，尝试切换「全帧/关键帧」模式。
...
视频文件
  │
  ▼ ffprobe -show_frames (流式读取 pts_time,width,height)
